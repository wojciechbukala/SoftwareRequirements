...
## Input data
- *packages.csv* - columns: [package_id, weight_kg, volume_m3, tw_open, tw_close, service_min, priority]
- *vehicles.csv*  - columns: [vehicle_id, max_weight_kg, max_volume_m3, depot_location_id]
- *locations.csv* - columns: [location_id, name]
- *distances.csv* - columns: [from_location_id, to_location_id, distance_km, travel_time_min]

## Output data
- *routes.csv* - columns: [vehicle_id, stop_order, location_id, package_id, arrival_time, departure_time]
- *undeliverable.csv* - columns: [package_id, reason]
- *summary.csv* - columns: [vehicle_id, total_distance_km, total_time_min, packages_delivered, packages_undeliverable]