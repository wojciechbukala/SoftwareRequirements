...
outputs_exist() {
    local folder="$1"
    [[ -d "$folder/output-Copilot-ScenarioA" ]] && \
    [[ -d "$folder/output-Copilot-ScenarioB" ]] && \
    [[ -d "$folder/output-Copilot-ScenarioC" ]]
}
...
process_run() {
    ...
    python3 "$REPO_ROOT/Verification/verify_run_copilot.py" "$run_folder" || true
}

[[ $# -ne 1 ]] && usage

if [[ "$1" == "--all" ]]; then
    for folder in "$REPO_ROOT"/Runs/Copilot*; do
        [[ -d "$folder" ]] || continue
        process_run "$folder"
    done
else
